...
bool Blockworks::AddToBlock(const BlockEntry &entry) {
    ...
    candidateBlock->entries.emplace_back(entry.tx, entry.txFee, entry.txSize,
                                         entry.txSigOps);
...
std::unique_ptr<BlockCandidate>
Blockworks::GetBlock(const CScript &scriptPubKeyIn) {
    ...
    if (IsMagneticAnomalyEnabled(*config, pindexPrev)) {
        ...
        std::sort(std::begin(candidateBlock->entries) + 1,
                  ...
                  [](const BlockEntry &a, const BlockEntry &b) -> bool {
...
void Blockworks::AddTransactionsToBlock(CTxMemPool &mempool) {
    ...
    for (const auto &entry : txPool) {
        ...
        while (!addStack.empty()) {
            ...
            candidateBlock->entries.emplace_back(entry.tx, entry.txFee,
                                                 entry.txSize, entry.txSigOps);
            nBlockSize += entry.txSize;
            ++nBlockTx;
            nBlockSigOps += entry.txSigOps;
            nFees += entry.txFee;
