...
void BlockAssembler::addPackageTxs(int &nPackagesSelected,
    ...
    pblocktemplate->entries.reserve(mempool->mapTx.size());
    for (const auto &mempoolEntry : mempool->mapTx.get<insertion_order>()) {
        // If tx already in block, skip.
        const auto txId = mempoolEntry.GetTx().GetId();
        if (inBlock.count(txId)) {
            // Shouldn't happen for priority txs.
            continue;
        }
        inBlock.insert(txId);

        auto blockSizeWithTx = nBlockSize + mempoolEntry.GetTxSize();
        auto maxBlockSigOps = GetMaxBlockSigOpsCount(blockSizeWithTx);
        if (blockSizeWithTx >= nMaxGeneratedBlockSize) {
            break;
        }
        ...
        if (nBlockSigOps + mempoolEntry.GetSigOpCount() >= maxBlockSigOps) {
            break;
        }

        pblocktemplate->entries.emplace_back(mempoolEntry.GetSharedTx(), mempoolEntry.GetFee(),
                            mempoolEntry.GetModifiedFee(),
                            mempoolEntry.GetTxSize(),
                            mempoolEntry.GetSigOpCount());
        nBlockSigOps += mempoolEntry.GetSigOpCount();
        nBlockSize += mempoolEntry.GetTxSize();
        nPackagesSelected += 1;
        ++nBlockTx;
    }
}
