...
void TestPackageSelection(const CChainParams &chainparams, CScript scriptPubKey,
    ...
    tx.vout.resize(1);
    tx.vout[0].nValue = Amount(5000000000LL) - Amount(1000);
    // This tx has a low fee: 1000 satoshis.
    // Save this txid for later use.
    uint256 hashParentTx = tx.GetId();
    mempool.addUnchecked(
        hashParentTx,
        entry.Fee(Amount(1000)).Time(GetTime()).SpendsCoinbase(true).FromTx(tx));
    ...
    mempool.addUnchecked(
        hashMediumFeeTx,
        entry.Fee(Amount(10000)).Time(GetTime()).SpendsCoinbase(true).FromTx(tx));
    ...
    mempool.addUnchecked(
        hashHighFeeTx,
        entry.Fee(Amount(50000)).Time(GetTime()).SpendsCoinbase(false).FromTx(tx));
    ...
    mempool.addUnchecked(hashLowFeeTx, entry.Fee(feeToUse + Amount(2)).FromTx(tx));
    pblocktemplate =
        BlockAssembler(config, chainparams).CreateNewBlock(scriptPubKey);
    ...
    tx.vout.resize(2);
    tx.vout[0].nValue = Amount(5000000000LL) - Amount(100000000);
    // 1BCC output.
