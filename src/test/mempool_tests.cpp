...
BOOST_AUTO_TEST_CASE(MempoolIndexingTest) {
    ...
    pool.addUnchecked(tx3.GetId(), entry.Fee(Amount(0LL)).Priority(100.0).FromTx(tx3));
    ...
    BOOST_CHECK_EQUAL(
        pool.CalculateMemPoolAncestors(entry.Fee(Amount(200000LL)).Time(4).FromTx(tx10),
                                       setAncestorsCalculated, 100, 1000000,
                                       1000, 1000000, dummy),
        true);
    BOOST_CHECK(setAncestorsCalculated == setAncestors);
...
BOOST_AUTO_TEST_CASE(MempoolSizeLimitTest) {
    ...
    tx1.vout[0].nValue = 10 * COIN;
    pool.addUnchecked(tx1.GetId(), entry.Fee(Amount(10000LL)).FromTx(tx1, &pool));
    ...
    pool.addUnchecked(tx2.GetId(), entry.Fee(Amount(5000LL)).FromTx(tx2, &pool));
    ...
    pool.addUnchecked(tx3.GetId(), entry.Fee(Amount(20000LL)).FromTx(tx3, &pool));
    ...
    pool.addUnchecked(tx4.GetId(), entry.Fee(Amount(7000LL)).FromTx(tx4, &pool));
    pool.addUnchecked(tx5.GetId(), entry.Fee(Amount(1000LL)).FromTx(tx5, &pool));
    pool.addUnchecked(tx6.GetId(), entry.Fee(Amount(1100LL)).FromTx(tx6, &pool));
    pool.addUnchecked(tx7.GetId(), entry.Fee(Amount(9000LL)).FromTx(tx7, &pool));
