...
static void SendMoney(const CTxDestination &address, Amount nValue,
...
static UniValue movecmd(const Config &config, const JSONRPCRequest &request) {
    ...
    Amount nAmount = AmountFromValue(request.params[2]).GetSatoshis();
    if (nAmount <= 0) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount for send");
    }
...
static UniValue sendmany(const Config &config, const JSONRPCRequest &request) {
    ...
    for (const std::string &name_ : keys) {
        ...
        CScript scriptPubKey = GetScriptForDestination(dest);
        Amount nAmount = AmountFromValue(sendTo[name_]).GetSatoshis();
        if (nAmount <= 0) {
            throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount for send");
        }
        totalAmount += nAmount;
