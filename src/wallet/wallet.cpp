...
bool CWallet::CreateTransaction(const std::vector<CRecipient> &vecSend,
        ...
        while (true) {
            ...
            for (const auto &recipient : vecSend) {
                ...
                if (txout.IsDust(dustRelayFee)) {
                    if (recipient.fSubtractFeeFromAmount && nFeeRet > Amount(0)) {
    ...
    if (GetBoolArg("-walletrejectlongchains",
        ...
        LockPoints lp;
        CTxMemPoolEntry entry(wtxNew.tx, Amount(0), 0, 0, 0, Amount(0), false, 0, lp);
...
std::map<CTxDestination, Amount> CWallet::GetAddressBalances() {
    ...
    for (std::pair<uint256, CWalletTx> walletEntry : mapWallet) {
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            ...
            Amount n =
                IsSpent(walletEntry.first, i) ? Amount(0) : pcoin->tx->vout[i].nValue;
